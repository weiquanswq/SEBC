   ...
   do                 
      ...
      do                         
        ...
        time $HADOOP_PATH/hadoop jar $HADOOP_MR/hadoop-examples.jar teragen \
                     ...
                     10000000  hdfs://node01/results/tg-10GB-${i}-${j}-${k} 1>tera_${i}_${j}_${k}.out 2>tera_${i}_${j}_${k}.err                       
        ...
        time $HADOOP_PATH/hadoop jar $HADOOP_MR/hadoop-examples.jar terasort \
	             ...
	             hdfs://node01/results/tg-10GB-${i}-${j}-${k}  \
                      hdfs://node01/results/ts-10GB-${i}-${j}-${k} 1>>tera_${i}_${j}_${k}.out 2>>tera_${i}_${j}_${k}.err                         
		
        $HADOOP_PATH/hadoop fs -rm -r -skipTrash /results/tg-10GB-${i}-${j}-${k}                         
        $HADOOP_PATH/hadoop fs -rm -r -skipTrash /results/ts-10GB-${i}-${j}-${k}
		echo "==end== Parameters(mapper-reducer-memory) ->  " ${i}-${j}-${k}
      done
   done
done
